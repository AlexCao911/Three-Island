...
const Home = () => {
  ...
  // Stage-based information display (inspired by JOSHUA's World)
  const renderStageInfo = () => {
    ...
    return (
      <div className="absolute top-24 left-1/2 transform -translate-x-1/2 z-10 text-center">
        <div className="bg-black/20 backdrop-blur-md rounded-2xl p-6 max-w-md mx-auto border border-gray-500/30 shadow-xl">
          <h2 className="text-2xl font-bold text-gray-100 mb-2">{content.title}</h2>
          <p className="text-gray-200 mb-3">{content.description}</p>
          <p className="text-sm text-gray-300">{content.action}</p>
        </div>
      </div>
    );
  };
